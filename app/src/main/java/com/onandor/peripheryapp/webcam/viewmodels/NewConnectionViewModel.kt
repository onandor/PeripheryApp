...
data class NewConnectionUiState(
    ...
    val tcpServerStopped: Boolean = false,
...
class NewConnectionViewModel @Inject constructor(
    ...
    private fun onTcpServerEvent(event: TcpServer.Event) {
        when (event) {
            ...
            TcpServer.Event.Stopped -> {
                _uiState.update { it.copy(tcpServerStopped = true) }
            }
    ...
    fun onStartTcpServer() {
        tcpServer.start()
        _uiState.update { it.copy(tcpServerStopped = false) }
    }
